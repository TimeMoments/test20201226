...
public class TestpaperServiceImpl implements ITestpaperService {
	...
	public List<Map<String, Object>> queryTestpaper() throws ClassNotFoundException, SQLException {
		String sql = "SELECT t.*,c.course_name FROM testpaper t LEFT "
				+ "JOIN course c ON t.course_id=c.course_id  order by"
				+ " t.testpaper_state,t.testpaper_time_begin  desc  ";
	...
	public List<Map<String, Object>> queryTestPaperRadioInfo(String id) throws ClassNotFoundException, SQLException {
		 String sql="SELECT  s.* FROM SUBJECT s WHERE subject_id   IN  "
		 		+ "(SELECT subject_id FROM testpaper_list WHERE testpaper_id=?)  "
		 		+ "AND subject_type='单选题'";
	...
	public void edit(String testpaper_id, String testpaper_name, String testpaper_radio_num, String testpaper_check_num,
			String testpaper_time_use, String classs_id, String course_id) throws ClassNotFoundException, FileNotFoundException, SQLException, IOException {
		String sql=" update testpaper set testpaper_name=?,testpaper_radio_num=?,"
				+ "testpaper_check_num=?,testpaper_time_use=?,classs_id=?,"
				+ "course_id=? where testpaper_id=? ";
		...
		Object[] values = new Object[9];
		values[0] = testpaper_name;
		values[1] = testpaper_radio_num;
		values[2] = testpaper_check_num;
		values[3] = testpaper_time_use;
		values[4] =classs_id;
		values[5] = course_id;
		values[6] =testpaper_id;
	...
	public void deleteByIdArray(String[] delIdArray) throws ClassNotFoundException, FileNotFoundException, SQLException, IOException {
		for(int i =0;i<delIdArray.length;i++){
			...
			String sql1 = " delete from testpaper_list where testpaper_id=? ";
			//根据试卷id在试卷表中删除试卷信息
			String sql2 = " delete from testpaper where testpaper_id=? ";
